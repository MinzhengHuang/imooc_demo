...
public class QQSlidingMenu extends HorizontalScrollView {
	 ...
	 * @param defStyle
	 */
	public QQSlidingMenu(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		// 获取我们定义的属性
		TypedArray ta = context.getTheme().obtainStyledAttributes(attrs,
				R.styleable.QQSlidingMenu, defStyle, 0);
		int n = ta.getIndexCount();
		for (int i = 0; i < n; i++) {
			int attr = ta.getIndex(i);
			switch (attr) {
			case R.styleable.QQSlidingMenu_rightPadding:
				...
				mMenuRightPadding = ta.getDimensionPixelSize(attr,
						(int) TypedValue.applyDimension(dip, 50, metrics));
				break;
			}
		}
		ta.recycle();//回收
		getWindowWidth(context);//获取屏幕的宽度
...
//		ViewHelper.setTranslationX(mMenu, mMenuWidth * scale * 1);
